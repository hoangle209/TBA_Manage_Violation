...
def set_config(size = (1728, 1728), patch_size = 576, conf_thresh = 0.15):
    size = size
    patch_size = patch_size
    conf_thresh = conf_thresh
    return size, patch_size, conf_thresh
    
def tile_inference_video_check_isolation(cam_id):
    color_dict = {
        0: (0, 138, 0),
        1: (255, 0, 0),
        2: (255, 255, 0),
    ...
    restricted_area = [] # TODO
    labels = [-1] # TODO

    size, patch_size, conf_thresh = set_config()
        ...
        if ret:
            ...
            for k in range(num_patches_w):
                for h in range(num_patches_h):
                    ...
                    img_i = _img[ymin:ymax, xmin:xmax]
                    r = model(img_i)
                    xyxy = r.xyxyn[0].cpu().numpy()
                    ...
                    for bb in xyxy:
                        ...
                        violate = _check_violate([cx, cy, bb[-1]], restricted_area, labels)
                        print('Warning') # TODO 
